% Bit Error Rate(BER) Versus Signal to Noise Ratio(SNR) for QAM 
clc;
clear all;
FRM = 2048;
MaxNumErrs = 200;
...
%%Initialization
%c = [1 1i -1];
Modulator = comm.GeneralQAMModulator(c);
AWGN = comm.AWGNChannel;
Demodulator = comm.GeneralQAMDemodulator(c);
...
for EbNo = EbNo_vector
    ...
    mod_sig = step(Modulator,u); % QAM Modulator
    % Channel
    rx_sig = step(AWGN,mod_sig); % AWGN Channel
    % Receiver
    y = step(Demodulator,rx_sig); % QAM Demodulator
...
grid on;
title('BER Versus EbNo - QAM Modulation')
xlabel('Eb/No (dB)');
ylabel('BER');
hold;
